class Movie < ActiveRecord::Base
  ...
  scope :top, -> { eager_load(:ratings).where(approved: true).select('movies.id, avg(ratings.score)').group('movies.id').order('avg(ratings.score) desc, movies.updated_at desc') }
